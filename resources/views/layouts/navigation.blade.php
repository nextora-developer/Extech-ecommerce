...
<div class="sm:hidden" x-data="{ openMoreSheet: false, openProfileSheet: false }">
    ...
    {{-- Profile Sheet --}}
    <div x-cloak x-show="openProfileSheet" class="fixed inset-0 z-[999] flex items-end justify-center"
        x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
        x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
        x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
        ...
        <div class="relative w-full max-w-lg px-4 pb-4" x-show="openProfileSheet"
            x-transition:enter="transition ease-out duration-300 transform"
            x-transition:enter-start="translate-y-full" x-transition:enter-end="translate-y-0"
            x-transition:leave="transition ease-in duration-200 transform" x-transition:leave-start="translate-y-0"
            x-transition:leave-end="translate-y-full">
            ...
            <div
                class="relative max-h-[100dvh] overflow-hidden rounded-[2.5rem] border border-white/10 bg-slate-900 shadow-[0_-20px_50px_rgba(0,0,0,0.5)]">

                {{-- Tech Accent Header --}}
                <div
                    class="shrink-0 px-6 pt-6 pb-5 flex items-center justify-between border-b border-white/5 bg-white/[0.01]">
                    <div>
                        <div class="flex items-center gap-2 mb-0.5">
                            <span
                        ...
                        <p class="text-lg font-bold text-white tracking-tight">
                            @auth
                                {{ explode(' ', auth()->user()->name)[0] }}'s Account
                            @else
                                Identity Access
                            @endauth
                        </p>
                ...
                {{-- Scrollable Body --}}
                <div class="overflow-y-auto max-h-[calc(100dvh-110px)]">
                    <div class="p-3">
                        @auth
                            {{-- Navigation Commands --}}
                            <div class="grid grid-cols-1 gap-1">
                                ...
                                @foreach ($navItems as $item)
                                    <a href="{{ route($item['route']) }}" @click="openProfileSheet = false"
                                        ...
                                        <span
                                            class="text-xs font-mono font-bold text-slate-300 group-hover:text-white tracking-widest">
                                            {{ $item['label'] }}
                                        </span>
                                ...
                                <form method="POST" action="{{ route('logout') }}">
                                    ...
                                    <button type="submit"
                                        ...
                                        <span class="text-xs font-mono font-bold text-rose-400 uppercase tracking-widest">
                                            Sign_Out
                                        </span>
                                    </button>
                                </form>
                    ...
                    {{-- Security Footer --}}
                    <div class="shrink-0 px-8 py-3 bg-black/40 border-t border-white/5 flex justify-center">
                        <div
