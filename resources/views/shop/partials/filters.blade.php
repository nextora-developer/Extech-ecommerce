...
@if ($mode === 'desktop')
    ...
    <div class="flex-1 min-h-0 flex flex-col">
        <div
            ...
            <div class="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-4 space-y-1.5">
                @php
...
@else
    ...
    <div class="space-y-4 pb-2">
        ...
        <div class="shrink-0 group/filter">
            <form id="{{ $formId }}" method="GET" action="{{ route('shop.index') }}"
                class="relative overflow-hidden bg-white/80 backdrop-blur-xl border border-black/[0.03] rounded-[2rem] p-4
                       shadow-[0_8px_30px_rgb(0,0,0,0.04)]">

                {{-- Animated Scanner Line Effect --}}
                <div
                    class="absolute top-0 left-0 w-full h-[2px] bg-gradient-to-r from-transparent via-[#15A5ED]/40 to-transparent">
                </div>
                ...
                <div class="flex items-center gap-4 mb-5">
                    ...
                    <div>
                        <h3 class="text-[13px] font-black text-slate-900 tracking-tight leading-none">Search Products</h3>
                        <p class="text-[9px] text-[#15A5ED] uppercase tracking-[0.2em] font-bold opacity-70">
                            Find what you need
                        </p>
                    </div>
                ...
                <div class="space-y-4">
                    @foreach (['q' => 'Search', 'sort' => 'Sort By'] as $name => $label)
                        <div class="relative">
                            <label
                                class="block text-[9px] uppercase tracking-[0.2em] text-slate-400 font-bold mb-1.5 ml-1">
                                {{ $label }}
                            </label>
        ...
        <div class="mt-0">
            <div
                class="bg-white border border-black/[0.03] rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)]
                       overflow-hidden flex flex-col">

                <div class="p-4 border-b border-slate-50 shrink-0 bg-white/50 backdrop-blur-md">
                    <div class="flex items-center gap-3">
                        <div class="h-2 w-2 rounded-full bg-[#15A5ED] animate-pulse"></div>
                        <h3 class="text-[13px] font-black text-slate-900 tracking-tight">Categories</h3>
                    </div>
                </div>
                ...
                <div class="p-3 space-y-1.5">
                    ...
                    <a href="{{ route('shop.index', $baseQuery) }}"
                        class="flex items-center justify-between rounded-2xl px-4 py-3 transition-all duration-300 group
                        {{ empty($activeCat) ? 'bg-slate-900 text-white ring-4 ring-slate-900/10' : 'text-slate-500 hover:bg-slate-50' }}">
                        <span class="text-[11px] font-black uppercase tracking-widest">All Products</span>
                        <span class="px-2 py-0.5 rounded-lg bg-[#15A5ED] text-[10px] text-white font-bold">
                            {{ $products->total() }}
                    ...
                    @isset($categories)
                        @foreach ($categories as $cat)
                            <a href="{{ route('shop.index', array_merge($baseQuery, ['category' => $cat->slug])) }}"
                                class="flex items-center justify-between rounded-2xl px-4 py-3 transition-all duration-300 border border-transparent
                                {{ $activeCat === $cat->slug ? 'bg-[#15A5ED]/10 border-[#15A5ED]/20 text-[#15A5ED]' : 'text-slate-600 hover:border-slate-100 hover:bg-slate-50/50' }}">
                                <span class="text-[13px] font-bold tracking-tight">{{ $cat->name }}</span>
                                <span class="text-[10px] font-mono opacity-40">
                                    [{{ str_pad((int) ($cat->products_count ?? 0), 2, '0', STR_PAD_LEFT) }}]
